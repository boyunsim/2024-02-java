...
public class StaticStudy {
    public static void main(String[] args) {
        ...
        Math.random(); //Math 클래스 안에 있는 random static method
        //Math math = new Math(); //
        //math.random();
        // 이렇게 두 줄을 통해 메소드를 호출할 필요가 없기 때문에 static 메소드가 사용이 편하다
        // 그리고 Math 클래스는 객체화를 할 수 없다. 객체화를 막기위해서 기본 생성자 접근레벨을 private로 세팅했기 때문에
    }
}
...
class StaticNumBox {
    ...
    }// 오버로딩은 리턴타입과는 아무런 상관이없다 (리턴 타입으로는 호출시 구분이 되지 않기 때문에)
